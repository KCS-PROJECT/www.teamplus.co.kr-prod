...
> **최종 업데이트**: 2026-07-18 (WEB-072 — dev-prod-sync 부분 동기화 누락으로 prod 빌드 `getAppVersionInfo` export 부재)
...
## 🟠 WEB-071: [Native WebView] 로그인 성공 전환 실패 후 무한 로딩·재로그인 잠김

### 발생 환경

- Flutter Native WebView에서 `/login` 로그인 성공 직후 대상 경로 전환이 지연되거나 완료되지 않는 경우
- 로그인 성공 뒤 뒤로가기로 `/login`에 다시 노출되는 경우

### 증상

- 로그인 성공 후 전체 화면 로더가 계속 표시됨
- 뒤로가기로 로그인 화면에 돌아오면 버튼이 `로그인 완료` 상태로 비활성화되어 재시도할 수 없음
- 로그인 성공 이동이 history에 `/login`을 남겨 이후 뒤로가기로 로그인 화면이 다시 노출될 수 있음

### 원인 분석

- Native 분기가 `window.__NEXT_ROUTER_PUSH__` 또는 `window.teamplusNavigate`를 사용해 로그인 성공 이동도 `router.push`로 처리함
- 성공 직후 `loginSuccess=true`, `isRedirecting=true`로 잠근 뒤 라우트 전환이 완료되지 않는 경우 상태를 되돌리는 경로가 없음
- 기존 오류 복구는 로그인 API의 `catch` 분기에만 있어, API 성공 후 네비게이션 실패에는 적용되지 않음

### 해결

- 로그인 성공 이동만 Web/Native 모두 `router.replace`로 통일해 `/login` history 잔존 제거
- `loginSuccess` 진입 후 8초 watchdog을 시작하고, 컴포넌트가 언마운트되지 않으면 로컬·전역 로더, 성공 상태, 중복 이동 잠금을 함께 초기화
- 타임아웃 안내 문구는 `MESSAGES.auth.navigationTimeout`으로 관리
- 일반 Native 화면 이동의 `push` 계약은 변경하지 않음

### 남은 범위

- 이 수정은 로그인 화면 잠김과 history 재진입을 완화하는 Web Phase 2 조치임
- access 만료 후 Middleware가 `/login`으로 리다이렉트하는 인증 쿠키 정합성 문제는 별도 근본 수정과 실기기 증거 수집이 필요함

### 관련 파일

- `teamplus-web/src/app/(auth)/login/page.tsx`
- `teamplus-web/src/lib/messages.ts`

---

---------------------------------------------------------------
        [Web · prod 작업트리] Written 2026.07.18. 18:29:30
---

---

## 🟠 WEB-072: [dev-prod-sync 부분 동기화 누락] prod 빌드 `has no exported member 'getAppVersionInfo'`

### Error Message

```
./src/app/(common)/mypage/page.tsx:20:10
Type error: Module '"@/services/native-bridge"' has no exported member 'getAppVersionInfo'.
Did you mean to use 'import getAppVersionInfo from "@/services/native-bridge"' instead?
Next.js build worker exited with code: 1
```

### 발생 환경

- **운영 작업트리(`www.teamplus.co.kr-prod`)의 Next.js build**에서만 발생 — 개발 작업트리는 정상
- dev/prod 는 공통 git 이력이 없는 독립 저장소로, 파일 단위 복사(dev-prod-sync)로만 반영됨

### Cause Analysis

- 2026-07-16 개발 저장소에서 `getAppVersionInfo`(+`AppVersionInfo` 타입)가 `native-bridge-ui.ts`에 신규 추가되고 `native-bridge.ts` 배럴에서 re-export 됨
- prod 반영 시 **호출부(`mypage/page.tsx`, `AppUpdatePrompt.tsx`)만 동기화되고 피호출부(`native-bridge.ts`, `native-bridge-ui.ts`)는 구버전으로 남아** import 대상이 존재하지 않게 됨
- 즉 코드 자체의 버그가 아니라 **선택 복사 시 import 체인 일부 누락**으로 인한 계약 불일치

### Incorrect Code

```typescript
// prod: src/app/(common)/mypage/page.tsx (신버전 — 동기화됨)
import { getAppVersionInfo } from "@/services/native-bridge";

// prod: src/services/native-bridge.ts (구버전 — 동기화 누락)
export { theme } from "./native-bridge-ui";
// ← getAppVersionInfo re-export 없음
```

### Correct Code

```typescript
// prod: src/services/native-bridge.ts (dev와 동일하게 동기화)
export { theme } from "./native-bridge-ui";
...
수정 방법: dev-prod-sync 스킬 선택 복사로 피호출부 2개 파일 반영

```bash
bash .claude/skills/dev-prod-sync/sync.sh --apply \
  teamplus-web/src/services/native-bridge.ts \
  teamplus-web/src/services/native-bridge-ui.ts
```

### Prevention Guide

- **선택 복사 시 파일 하나가 아니라 import 체인 단위로 반영**: 화면(page/component)을 반영하면 그 파일이 새로 import 하는 서비스/유틸(`@/services/*`, `@/lib/*`)의 export 존재 여부를 prod 쪽에서 grep 으로 확인
- 반영 후 검증: prod 에는 node_modules 가 없으므로(빌드는 Jenkins) **dev 쪽에서 `npx tsc --noEmit` 0 에러 확인 + 관련 파일 dev↔prod `diff -q` 동일성 확인**으로 대체
- `Did you mean to use 'import X from ...' instead?` 힌트가 뜨면 named export 부재(배럴 re-export 누락) 신호 — 배럴 파일(`native-bridge.ts` 등)의 버전 드리프트부터 의심

### 관련 파일

- `teamplus-web/src/services/native-bridge.ts` — 배럴 re-export 누락(원인)
- `teamplus-web/src/services/native-bridge-ui.ts` — `getAppVersionInfo` 본체 누락(원인)
- `teamplus-web/src/app/(common)/mypage/page.tsx` — 호출부(증상 발현 지점)
- `teamplus-web/src/components/common/AppUpdatePrompt.tsx` — 호출부
- `.claude/skills/dev-prod-sync/` — 반영 도구

### 재발 사례 및 전수 해소 (2026.07.18. 19:05 추가)

동일 원인 클래스의 2차 증상이 prod Jenkins 빌드에서 재발:

```
./src/hooks/useAppBack.ts:98:17
Type error: Property 'clearSession' does not exist on type 'AuthContextValue'.
```

- `useAppBack.ts`(신버전, dev와 동일)는 동기화됐으나 `AuthContext.tsx`(구버전, `clearSession` 부재)는 누락 — WEB-072와 동일한 부분 동기화 드리프트
- 전수 조사 결과 **`teamplus-web/src`에서 142개 파일 드리프트** 확인 → 개별 수정 대신 전수 해소로 전환

**전수 해소 절차 (표준 플레이북)**:

1. **드리프트 전수 목록**: `diff -rq {dev}/teamplus-web/src {prod}/teamplus-web/src`
2. **방향 판별 (핵심)**: prod 쪽 각 파일의 blob 해시가 dev git 이력에 존재하는지 확인 — 존재하면 "dev 과거 버전"(덮어쓰기 안전), 없으면 운영 핫픽스 의심
   ```bash
   sha=$(git hash-object "$PROD_FILE"); git -C "$DEV" cat-file -e "$sha" && echo STALE || echo UNKNOWN
   ```
3. 이번 사례: 142개 중 141개 STALE(안전) · 1개 UNKNOWN(`bridge-security.ts`) → 수동 diff 결과 **prod에만 운영 도메인 `teamplusweb.icetimes.co.kr` origin 허용 4줄 존재(운영 핫픽스 미백포트)** — dev로 덮으면 운영 Bridge origin 검증 파괴
4. **역방향 백포트 먼저**: `sync.sh --reverse --apply teamplus-web/src/services/bridge-security.ts` (운영 핫픽스를 dev로 회수)
5. **정방향 전체 동기화**: `sync.sh --apply teamplus-web/src` (+ stale 판별된 `teamplus-backend/src/menus/menu-config.json`)
6. **검증**: `diff -rq` 잔여 0건 + dev `npx tsc --noEmit` 0 에러
7. **불가침 확인**: prod 전용 `teamplus-backend/src/payments/settlement/`(dev에 없음)은 `--delete` 미사용으로 보존

**Prevention 강화**: prod 반영은 파일 단위 선별이 아니라 **서브프로젝트 `src` 디렉토리 단위 동기화를 기본**으로 하고, 동기화 전 blob 해시 방향 판별로 운영 핫픽스 유실을 차단한다.
