...
export function ensureFile(filePath: string): void {
  ...
  // 끊긴 심볼릭 링크(타깃 부재 — 예: 다른 머신 절대경로 잔재)면 먼저 제거한다.
  // existsSync 는 broken symlink 를 false 로 보므로, 제거하지 않으면 아래 openSync("a") 가
  // 끊긴 링크에서 ENOENT 로 실패한다.
  try {
    if (fs.lstatSync(filePath).isSymbolicLink() && !fs.existsSync(filePath)) {
...
export function updateCurrentSymlink(
  ...
  try {
    // lstatSync 로 검사 — existsSync 는 끊긴 심볼릭 링크를 false 로 봐서 unlink 가 누락되고,
    // 이어지는 symlinkSync 가 EEXIST → 폴백 openSync 에서 ENOENT 로 실패한다.
