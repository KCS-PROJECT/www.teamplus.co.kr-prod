...
async function main() {
  ...
  // ========================================
  // 앱 약관 기본 데이터
  // [2026-06-13] 약식 본문은 isActive: false 로 시드 — 웹 /terms 의 상세 처리방침
  // fallback(policy-content.ts)을 가려 앱 심사(Data Safety 대조) 리젝 사유가 되지 않게 한다.
  // 마케팅 1.1.0만 동의 증빙에 필요하므로 웹 현행 전문과 동일한 본문으로 게시한다.
  //
  // ⚠️ 게시 판정은 시행일 기반이다 — isActive 를 true 로 바꾸는 것만으로는 게시되지 않는다.
  //   현행 = isActive=true AND publishedAt <= now 인 것 중, 그 유형에서 시행일이 가장 늦은 1건.
  //   isActive 는 "무효화되지 않음"을 뜻하는 필요조건일 뿐이다(취소 표시).
  //   최종본은 어드민 "약관 및 정책 관리"에서 버전·시행일과 함께 등록한다.
  // ========================================
  const termsData = [
    {
...
- SMS·푸시·이메일 수신 거부: 앱/웹 "알림 설정" 또는 고객센터(02-557-5321)로 요청 (무료)
...
`,
      version: "1.1.0",
      isActive: true,
      publishedAt: new Date("2026-07-29T15:00:00.000Z"),
    },
  ];
