...
function StarRating({
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <div className="flex items-center gap-2" role="radiogroup" aria-label="만족도 별점">
        {[1, 2, 3, 4, 5].map((star) => (
          <button
            ...
            className="p-1 rounded-w-pill transition-transform motion-reduce:transition-none active:scale-90 focus:outline-none focus-visible:ring-2 focus-visible:ring-ice-500/50"
          >
            <Icon
              name={star <= active ? 'star' : 'star_border'}
              className={cn(
                'text-4xl transition-colors motion-reduce:transition-none',
                star <= active ? 'text-amber-400' : 'text-wtext-4 dark:text-rink-500'
              )}
              aria-hidden="true"
            />
          </button>
      ...
      {active > 0 && (
        <p className="text-card-body font-semibold text-ice-500" aria-live="polite">
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <span className="text-card-body font-bold text-wtext-1 dark:text-white">사진 첨부</span>
        <span className="text-card-meta text-wtext-3 dark:text-rink-300">최대 {MAX_PHOTOS}장</span>
      </div>
      <div className="flex gap-2 flex-wrap">
        ...
        {photos.length < MAX_PHOTOS && (
          <button
            ...
            className="w-20 h-20 rounded-xl border-2 border-dashed border-wline dark:border-rink-700 flex flex-col items-center justify-center gap-1 hover:bg-wbg dark:hover:bg-rink-800 transition-colors motion-reduce:transition-none focus:outline-none focus-visible:ring-2 focus-visible:ring-ice-500/50"
            aria-label="사진 추가"
          >
            <Icon name="add_a_photo" className="text-2xl text-wtext-3 dark:text-rink-300" aria-hidden="true" />
        ...
        {photos.map((src, i) => (
          <div key={i} className="relative w-20 h-20">
            ...
            <button
              type="button"
              onClick={() => onRemove(i)}
              className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-w-pill bg-rink-700 dark:bg-rink-500 flex items-center justify-center hover:bg-red-500 transition-colors motion-reduce:transition-none"
...
function ClassInfoCard({
  ...
  return (
    <div className="bg-white dark:bg-rink-800 rounded-xl p-4 border border-wline-2 dark:border-rink-700 flex items-center gap-4 shadow-sm">
      <div className="w-14 h-14 rounded-xl bg-wline-2 dark:bg-rink-700 flex items-center justify-center shrink-0">
        <Icon name="sports_hockey" className="text-2xl text-wtext-3 dark:text-rink-300" aria-hidden="true" />
      </div>
      <div className="flex-1 min-w-0">
        <h3 className="text-card-emphasis font-bold text-wtext-1 dark:text-white truncate">{cls}</h3>
        <p className="text-card-body text-ice-500 font-medium mt-0.5">
...
function ReviewWriteInner() {
  ...
  return (
    <MobileContainer hasBottomNav>
      ...
      {/* ─── 본문 ──────────────────────────────────────── */}
      <main
        className="flex-1 overflow-y-auto hide-scrollbar pb-30"
        role="main"
        aria-label="리뷰 작성 폼"
      >
        <div className="flex flex-col gap-6 px-5 pt-5">
          ...
          {/* 제출 오류 메시지 */}
          {submitError && (
            <div className="flex items-center gap-2 px-4 py-3 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800" role="alert">
              <Icon name="error_outline" className="text-red-500 text-card-title shrink-0" aria-hidden="true" />
              <p className="text-card-body text-red-600 dark:text-red-400">{submitError}</p>
            </div>
          )}
          ...
          >
            <div className="flex items-center justify-center gap-2 mb-1">
              <span className="text-[11px] font-bold uppercase tracking-wider text-ice-500 bg-blue-50 dark:bg-blue-900/20 px-2 py-1 rounded">
                STEP 1 · 필수
              </span>
            </div>
            <h2
              id="rating-heading"
              className="text-card-title font-bold text-wtext-1 dark:text-white text-center mt-3 mb-5"
            >
              만족도는 어떠셨나요?
            </h2>
          ...
          <section aria-labelledby="review-heading">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 id="review-heading" className="text-card-body font-bold text-wtext-1 dark:text-white">
                상세 후기
              </h2>
              <span className="text-[11px] font-medium text-wtext-3 dark:text-rink-300 px-2 py-0.5 rounded bg-wline-2 dark:bg-rink-800">
                선택
              </span>
            </div>
            <div className="relative">
              <textarea
                ...
                rows={6}
                className="w-full px-4 py-3 pb-10 bg-white dark:bg-rink-800 border border-wline dark:border-rink-700 rounded-xl text-card-body text-wtext-1 dark:text-white placeholder-wtext-3 resize-none focus:outline-none focus:ring-2 focus:ring-ice-500/20 focus:border-ice-500 transition-colors motion-reduce:transition-none"
                aria-label="상세 후기 입력"
              />
              <span
                className="absolute bottom-3 right-4 text-card-meta text-wtext-3 dark:text-rink-300 tabular-nums"
                aria-live="polite"
              >
          ...
          <section className="rounded-xl bg-white dark:bg-rink-800 border border-wline-2 dark:border-rink-700 p-4 shadow-sm">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <ul className="space-y-1 text-card-meta text-wtext-3 dark:text-rink-300">
                  <li className="flex items-start gap-1.5">
                    <span className="text-wtext-4 dark:text-rink-500 mt-0.5" aria-hidden="true">•</span>
      ...
      <div className="fixed bottom-fab-safe left-1/2 -translate-x-1/2 w-full max-w-[var(--mobile-shell-width,28rem)] px-4 z-40">
        <button
          ...
          className={cn(
            'w-full h-14 rounded-2xl font-bold text-card-emphasis transition-all motion-reduce:transition-none active:brightness-95 shadow-sh-1',
