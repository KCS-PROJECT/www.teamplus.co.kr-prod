...
function MonthSection({
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-wline-2 bg-white shadow-sm dark:border-rink-700 dark:bg-rink-800">
      <button
        type="button"
        onClick={onToggle}
        aria-expanded={expanded}
        aria-controls={panelId}
        className="flex w-full items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-wline-2/40 active:brightness-95 motion-reduce:transition-none dark:hover:bg-rink-700/40"
      >
        <span className="tabular-nums flex size-9 shrink-0 items-center justify-center rounded-w-pill bg-ice-500/10 ring-2 ring-ice-500/5 text-card-body font-bold text-ice-500 dark:bg-blue-900/30">
          {month.month}
        </span>
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="tabular-nums text-card-body font-semibold text-wtext-2 dark:text-rink-100">
              {MESSAGES.calendar.historyCount(count)}
            </span>
          </div>
          {/* 막대 그래프 — 월별 출석 횟수 추이 */}
          <div className="mt-2 h-1.5 overflow-hidden rounded-w-pill bg-wline-2 dark:bg-rink-700">
            <div
              className="h-full rounded-w-pill bg-ice-500 transition-all duration-300 motion-reduce:transition-none"
        ...
        <Icon
          name="expand_more"
          className={cn(
            'shrink-0 text-xl text-wtext-3 transition-transform motion-reduce:transition-none dark:text-rink-300',
            expanded && 'rotate-180',
          )}
          aria-hidden="true"
        />
      ...
      {expanded && (
        <div id={panelId} className="px-4 pb-3">
          {items.length > 0 ? (
            ...
            <p className="py-3 text-center text-card-meta text-wtext-3 dark:text-rink-300">
  ...
  return (
    <div className="flex items-center justify-between px-1">
      <button
        type="button"
        onClick={onPrev}
        className="flex size-10 items-center justify-center rounded-w-pill transition-colors hover:bg-wline-2 active:brightness-95 motion-reduce:transition-none dark:hover:bg-rink-800"
      ...
      <div className="flex items-center gap-2">
        <h2 className="tabular-nums text-card-title font-bold text-wtext-1 dark:text-white">
        ...
        {currentYear !== thisYear && (
          <button
            type="button"
            onClick={onToday}
            className="rounded-lg px-2 py-1 text-card-meta font-semibold text-ice-500 transition-colors hover:bg-blue-50 motion-reduce:transition-none dark:hover:bg-blue-900/20"
          >
      ...
      <button
        ...
        onClick={onNext}
        disabled={isFuture}
        className="flex size-10 items-center justify-center rounded-w-pill transition-colors hover:bg-wline-2 active:brightness-95 disabled:cursor-not-allowed disabled:opacity-30 disabled:hover:bg-transparent motion-reduce:transition-none dark:hover:bg-rink-800"
    ...
    <MobileContainer hasBottomNav>
      ...
      <main className="flex-1 overflow-y-auto hide-scrollbar bg-wbg dark:bg-rink-900">
        <div className="flex flex-col gap-4 px-4 py-4">
          {/* 연도 네비게이션 */}
          <div className="rounded-xl border border-wline-2 bg-white py-2 shadow-sm dark:border-rink-700 dark:bg-rink-800">
            <YearNavigation
          ...
          {isLoading ? null : (
            <>
              {/* 연간 총계 — "올해 출석 N회" 단일 타일 */}
              <section
                aria-label={MESSAGES.calendar.historyTotal}
                className="flex items-center gap-4 rounded-2xl border border-wline-2 bg-white p-5 shadow-sm dark:border-rink-700 dark:bg-rink-800"
              >
                ...
                <div className="flex flex-col">
                  ...
                  <span className="tabular-nums text-w-h2 font-extrabold text-wtext-1 dark:text-white">
              ...
              {hasPresent ? (
                ...
                <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-wline-2 bg-white p-10 text-center dark:border-rink-700 dark:bg-rink-800">
