...
/**
 * 장비 점검 조회 (학생 — TEEN/CHILD)
 *
 * - 자기 소속 팀의 점검 이력만 read-only 조회
 * - 상세 페이지로 진입 가능 (수정/삭제 권한 없음)
 *
 * Backend:
 *   - GET /api/v1/teams/my/list           → 내 소속 팀
 *   - GET /api/v1/equipment-inspections/teams/:teamId
 */
...
export default function StudentEquipmentInspectionPage() {
  const { user } = useSessionAuth();
  const { navigate } = useNavigation();
  const { toast } = useToast();
  const [teams, setTeams] = useState<MyTeamItem[] | null>(null);
  ...
  // 어린이(CHILD) 화면 — WCAG AAA 가독성 (큰 텍스트/간격)
  ...
  // 1) 내 소속 팀 목록 로드 (학생/공통 엔드포인트)
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const res = await api.get<MyTeamItem[]>('/teams/my/list');
      if (cancelled) return;
  ...
  return (
    <MobileContainer hasBottomNav>
      ...
      <main className={cn(
        'flex-1 overflow-y-auto px-4 pt-3 pb-24 hide-scrollbar',
        isChild && 'text-card-title-child leading-relaxed',
      )}>
        {/* 빈 상태 */}
          ...
          <div className="mb-3 flex gap-2 overflow-x-auto hide-scrollbar">
            {teams.map((t) => (
              <button
                ...
                className={cn(
                  'shrink-0 rounded-w-pill px-3 py-1.5 font-bold transition-colors',
                  isChild ? 'min-h-[48px] text-card-title-child' : 'text-card-body',
                  selectedTeamId === t.id
        ...
        {teams && teams.length > 0 && (
          <>
            <div className="mb-3 grid grid-cols-3 gap-2">
              {(['pending', 'completed', 'issue_found'] as InspectionStatus[]).map((s) => (
                <div
                  key={s}
                ...
                >
                  ...
                  <div className={cn(
                    'mt-1 font-extrabold text-wtext-1 dark:text-white tabular-nums',
                    isChild ? 'text-w-h2' : 'text-card-section',
                  )}>
                    {statusCounts[s]}
                  </div>
                </div>
            ...
            <div className="mb-3 flex gap-2">
              {(['all', 'pending', 'completed', 'issue_found'] as const).map((f) => (
                <button
                  ...
                  onClick={() => setFilter(f)}
                  className={cn(
                    'flex-1 rounded-w-md py-2 font-bold transition-colors',
                    isChild ? 'min-h-[48px] text-card-title-child' : 'text-card-body',
                    filter === f
                      ? 'bg-ice-500 text-white'
                      : 'bg-wsurface dark:bg-rink-800 text-wtext-2 dark:text-rink-100 border border-wline dark:border-rink-700',
              ...
              <ul className="space-y-2">
                {inspections.map((ins) => (
                  <li key={ins.id}>
                    <button
                      type="button"
                      onClick={() => navigate(`/student-equipment-inspection/${ins.id}`)}
                      className={cn(
                        'w-full rounded-w-md bg-wsurface dark:bg-rink-800 border border-wline dark:border-rink-700 text-left transition-colors hover:bg-wline-2/40 dark:hover:bg-rink-700/40',
                        isChild ? 'p-5 min-h-[72px]' : 'p-4',
                      )}
                    >
                      <div className="flex items-center justify-between gap-2">
                        <span
                          className={cn(
                            'rounded-w-pill px-2 py-0.5 font-bold',
                            STATUS_COLOR[ins.status],
                            isChild ? 'text-card-title-child' : 'text-card-meta',
                          )}
                        >
