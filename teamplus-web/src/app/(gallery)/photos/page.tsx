...
// 카테고리별 accent dot (summary·선택 인디케이터) — flat: 정규/수업=it-blue, 경기=it-red(SoT §4)
...
export default function PhotoGalleryPage() {
  ...
  return (
    <MobileContainer hasBottomNav={true}>
      ...
      {/* Body — 에디토리얼 아카이브 (flat: 회색 캔버스 + 흰 섹션) */}
      <main
        className="flex-1 overflow-y-auto hide-scrollbar relative bg-it-canvas dark:bg-puck"
        role="main"
        aria-label="포토 갤러리"
      >
        {isLoading ? null : photos.length > 0 ? (
          <>
            {/* ─── Summary stats — hero number (흰 섹션) ─── */}
            <section className="bg-it-surface dark:bg-it-blue-950 px-4 pt-6 pb-5" aria-label="갤러리 요약">
              <div className="flex items-baseline gap-1.5">
                <span className="text-w-display font-bold tracking-[-0.03em] tabular-nums leading-none text-it-ink-800 dark:text-white">
                  {photos.length}
                </span>
                <span className="text-card-body font-medium text-it-ink-500 dark:text-rink-300 tracking-tight">
                  장의 기록
                </span>
              </div>
              <div className="mt-3 flex items-center gap-3 text-card-meta text-it-ink-500 dark:text-rink-300">
                <span className="inline-flex items-center gap-1.5 tabular-nums">
                  ...
                  <span className="font-bold text-it-ink-800 dark:text-rink-100">{classCount}</span>
                </span>
                ...
                <span className="inline-flex items-center gap-1.5 tabular-nums">
                  ...
                  <span className="font-bold text-it-ink-800 dark:text-rink-100">{matchCount}</span>
                </span>
              </div>
            </section>

            {/* ─── Filter tabs — 인라인 밑줄 (sticky, 흰 섹션 hairline) ─── */}
            <nav
              className="sticky top-0 z-20 px-4 flex items-center gap-6 h-11 bg-it-surface dark:bg-it-blue-950 border-b border-it-line dark:border-rink-800"
              role="tablist"
              aria-label="갤러리 필터"
            >
              {FILTER_TABS.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'relative inline-flex items-baseline gap-1.5 h-full text-card-body font-semibold whitespace-nowrap transition-colors motion-reduce:transition-none focus:outline-none focus-visible:text-it-blue-500 dark:focus-visible:text-it-blue-300',
                      active
                  ...
                  >
                    ...
                    <span
                      className={cn(
                        ...
                        active ? 'text-it-blue-500 dark:text-it-blue-300' : 'text-it-ink-500 dark:text-rink-300',
                      )}
                    >
                      {counts[tab.key]}
                    </span>
            ...
            {/* ─── Month groups — 아카이브 스타일 (흰 섹션, 섹션 간 8px 회색 갭) ─── */}
            {monthGroups.length > 0 ? (
              <div>
                {monthGroups.map((group) => (
                  <section key={group.month} className="mt-2 bg-it-surface dark:bg-it-blue-950 pb-6" aria-label={group.month}>
                    {/* Month header — "04 · 2026" archive marker */}
                    <header className="flex items-end justify-between gap-3 px-4 pt-5 pb-3">
                      <div className="flex items-baseline gap-2">
                        <span className="text-w-h2 font-bold tracking-[-0.03em] tabular-nums leading-none text-it-ink-800 dark:text-white">
                          {String(group.monthNum).padStart(2, '0')}
                        </span>
                        <span className="text-it-ink-300 dark:text-wtext-2 select-none text-card-meta" aria-hidden="true">·</span>
                        <span className="text-card-meta font-semibold text-it-ink-500 dark:text-rink-300 tracking-[0.14em] uppercase">
                          {group.year}
                        </span>
                      </div>
                      <span className="text-card-meta font-medium text-it-ink-500 dark:text-rink-300 tabular-nums tracking-tight">
                        {MESSAGES.gallery.photoCountLabel(group.photos.length)}
                      </span>
                    </header>

                    {/* 3-col grid — 사진 그리드는 비주얼 결과물(이미지)이라 PhotoTile 골격 유지 (공유 컴포넌트 iceTheme 미지원) */}
              ...
              // 필터 결과만 empty (전체는 photos.length > 0) — 흰 섹션
              <div className="mt-2 bg-it-surface dark:bg-it-blue-950 flex flex-col items-center justify-center py-20 px-5 text-center">
                <Icon
                  name="filter_alt_off"
                  className="text-[38px] text-it-ink-300 dark:text-wtext-2 mb-4"
                  aria-hidden="true"
                />
                <p className="text-card-body font-semibold text-it-ink-800 dark:text-rink-100 tracking-tight mb-1">
                  해당 분류의 사진이 없습니다
                </p>
          ...
          // 전체 empty state — 흰 섹션
          <div className="mt-2 bg-it-surface dark:bg-it-blue-950 flex flex-col items-center justify-center py-24 px-6 text-center">
            <Icon
              name="photo_library"
              className="text-[44px] text-it-ink-300 dark:text-wtext-2 mb-5"
              aria-hidden="true"
            />
            <h2 className="text-card-title font-bold text-it-ink-800 dark:text-rink-100 tracking-tight mb-2">
              {MESSAGES.gallery.emptyPhotos}
            </h2>
            <p className="text-card-meta text-it-ink-500 dark:text-rink-300 leading-[1.6] max-w-[260px]">
              수업과 경기 순간이 쌓이면
              <br />
              이곳에 월별로 정리됩니다
            </p>
      ...
      {isSelectMode && selectedPhotos.size > 0 && (
        ...
        >
          <div className="bg-it-surface dark:bg-rink-800 rounded-w-lg shadow-sh-3 border border-it-line dark:border-rink-700 p-4">
            <div className="flex items-center justify-between">
              <span className="text-card-body font-bold text-it-ink-800 dark:text-rink-100">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-4 py-2 bg-it-fill dark:bg-rink-700 text-it-ink-800 dark:text-rink-100 rounded-w-md text-card-body font-bold hover:bg-it-line dark:hover:bg-rink-500 transition-colors motion-reduce:transition-none flex items-center"
                >
      ...
      <button
        ...
        className={cn(
          'fixed right-4 z-30 flex h-14 w-14 items-center justify-center rounded-w-pill shadow-sh-3 transition-colors active:brightness-95 motion-reduce:transition-none focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-rink-900',
          isSelectMode
            ? 'bg-it-blue-800 hover:bg-it-blue-900 text-white focus-visible:ring-it-blue-500/30 dark:bg-rink-700 dark:hover:bg-rink-500'
            : 'bg-it-blue-500 hover:bg-it-blue-600 text-white focus-visible:ring-it-blue-500'
        )}
        style={{
          bottom:
