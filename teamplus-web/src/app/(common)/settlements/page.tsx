...
const SETTLEMENT_STATUS: Record<SettlementStatus, { label: string; className: string }> = {
  ...
  processing: {
    label: '처리 중',
    className: 'bg-blue-100 text-ice-500 dark:bg-blue-900/20 dark:text-blue-400',
  },
};
...
const SETTLEMENT_STATUS_FALLBACK: { label: string; className: string } = {
  label: '미정',
  className: 'bg-wline-2 text-wtext-3 dark:bg-rink-700 dark:text-rink-300',
};
...
function TransactionCard({ tx }: { tx: Transaction }) {
  ...
  return (
    <article className="bg-white dark:bg-rink-800 rounded-xl border border-wline-2 dark:border-rink-700 p-4 transition-colors motion-reduce:transition-none">
      {/* Header: 주문번호 + 상품명 */}
      <div className="flex items-start justify-between mb-3">
        <div className="min-w-0 flex-1">
          <p className="text-card-meta font-mono text-wtext-3 dark:text-rink-300 mb-0.5">
            {tx.orderNumber}
          </p>
        ...
        <span className={`shrink-0 ml-3 inline-flex items-center px-2 py-0.5 rounded-md text-card-meta font-bold ${statusCfg.className}`}>
          {statusCfg.label}
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div>
          <p className="text-card-meta text-wtext-3 dark:text-rink-300 font-medium mb-0.5">
            결제 일시
          </p>
          <p className="text-card-meta font-medium text-wtext-2 dark:text-rink-100 tabular-nums">
            {date}
            {time && <span className="text-wtext-3 dark:text-rink-300 ml-1">{time}</span>}
          </p>
        </div>
        <div>
          <p className="text-card-meta text-wtext-3 dark:text-rink-300 font-medium mb-0.5">
          ...
          <p className="text-card-meta font-medium text-wtext-2 dark:text-rink-100">
            {tx.paymentMethod}
          </p>
        </div>
        <div>
          ...
          <p className="text-card-body font-bold text-wtext-1 dark:text-white tabular-nums text-right">
        ...
        <div className="text-right">
          ...
          <p className="text-card-body font-bold text-ice-500 tabular-nums">
      ...
      <div className="flex items-center justify-between pt-3 border-t border-wline-2 dark:border-rink-700">
        <span className="text-card-meta text-red-600 dark:text-red-400 font-medium">
          수수료 -{formatCurrency(tx.fee)}
        </span>
        <button
          type="button"
          className="text-ice-500 text-card-meta font-semibold flex items-center gap-0.5 hover:underline"
        >
          상세 보기
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-20">
      <div className="flex items-center justify-center size-16 rounded-2xl bg-wline-2 dark:bg-rink-800 mb-4">
        <Icon name="receipt_long" className="text-3xl text-wtext-4 dark:text-rink-500" />
      </div>
    ...
    <MobileContainer hasBottomNav>
      ...
      <main className="flex-1 overflow-y-auto hide-scrollbar bg-wbg dark:bg-rink-900">
        ...
        <MonthNavigator
          ...
          className="bg-white dark:bg-rink-800 border-b border-wline-2 dark:border-rink-700"
        />
        ...
        <div className="p-4 flex flex-col gap-4">
          ...
          <section>
            ...
            {visibleCount < transactions.length && (
              <button
                ...
                className="mt-3 w-full py-3 rounded-xl border border-dashed border-wline dark:border-rink-700 text-card-body font-semibold text-wtext-3 dark:text-rink-300 hover:bg-wbg dark:hover:bg-rink-800 active:brightness-95 transition-colors motion-reduce:transition-none"
              >
                거래 내역 더보기
              </button>
            )}
          ...
          <button
            ...
            disabled
            className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl bg-ice-500 text-white font-semibold text-card-body hover:bg-ice-700 active:brightness-95 transition-colors motion-reduce:transition-none disabled:opacity-50 disabled:cursor-not-allowed"
          >
