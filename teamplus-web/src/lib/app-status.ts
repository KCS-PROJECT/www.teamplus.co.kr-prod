 ...
 *
 * 정책 (2026-06-07 최초 · 2026-06-15 전 인증화면 표시 전환 · 2026-07-18 로그인 숨김 전환):
 *   - 로그인(`/login` + 역할별 `/login/{role}`): 상태바 **숨김** — 2026-07-18 사용자 직접
 *     지시(appstatus 영역 숨김 + 배경을 로그인 body 색으로 통일). 색 통일은 `useAuthUI` 가
 *     scaffoldBackgroundColor 로 전송한다.
 *   - 그 외 화면: 상태바 **표시** (회원가입/계정찾기 포함 — 2026-06-15 사용자 직접 지시).
...
/** 상태바를 숨길 경로(접두 일치).
 *  [2026-07-18 사용자 직접 지시] 로그인 화면은 appstatus 숨김 + body 색 통일.
 *  (prefix 일치라 `/login/{role}` 역할별 로그인도 포함된다.)
 *  회원가입·계정찾기는 2026-06-15 지시대로 **표시** 유지. (splash/onboarding 은 아래 SKIP 별도.) */
export const APP_STATUS_HIDDEN_PREFIXES = ["/login"] as const;
